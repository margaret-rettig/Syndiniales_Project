...
t.test(surface_asv_counts, depth_asv_presence, paired = FALSE)
```
	Welch Two Sample t-test
data:  surface_asv_counts and depth_asv_presence
t = -5.5731, df = 30.585, p-value = 4.338e-06
...
print(epoch1_mean)
```
Epoch Means: 154 115 139 127 122 117 143 138 163 125
Average Epoch 1 Mean: 134.3
...
print(epoch2_mean)
```
Epoch 2 Mean ASV Counts: 172 142 120 120  81 112 127 193 210  85 121 109 161
Epoch 2 Average ASV Count: 134.8462
```{r}
sd(epoch2_counts)
```
...
print(epoch3_mean)
```
Epoch 3 Sample ASV Presence: 159 141 123 127 123 177 149 164 115 132 185 162 132 130 207 134 150 161 175 177 132 134 145 141 122 130 130 136 129 124 147 130
Epoch 3 Average ASV Count: 144.4688
```{r}
sd(epoch3_counts)
```
...
epoch        2   1580     790    1.16  0.321
Residuals   52  35410     681

P value is greater then the significance level of 0.05, so we support the null hypothesis. The difference between the means is not statistically significant.
...
```{r}
a <- boxplot(surf_sum, depth_sum, main = "ASV Presence Variation Across Depths", names = c("Surface - ML", "ML - 100M"), horizontal = TRUE, xlab = "Number of ASVs Present", ylab = "Depth")
```
...
##Venn Diagram
**Jaccard Index Surface Samples vs. Depth Samples**
```{r}

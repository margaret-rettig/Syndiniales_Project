...
```{r}
for(n in 1:length(samples$depth_layer)){
  if (samples$depth_layer[n] == "ML-100m"){
    samples$depth_layer[n] = "ML-75m"
  }
}
for(i in 1:length(samples$layer_epoch)){
  if(samples$layer_epoch[i] == "E1_ML-100m"){
    samples$layer_epoch[i] = "E1_ML-75m"
  } else if(samples$layer_epoch[i] == "E2_ML-100m"){
    samples$layer_epoch[i] = "E2_ML-75m"
  } else if(samples$layer_epoch[i] == "E3_ML-100m"){
    samples$layer_epoch[i] = "E3_ML-75m"
  } 
}
head(samples)
```
...
```{r}
#remove sample 46 -> only one with depth > 75m 
samples <- samples[-45,]
```
...
clades_df <- taxa_df %>% 
  ...
  arrange(Family) #arrange number in decreasing order
```
...
#   psmelt() %>% 
#   arrange(Order)
my_palette <- brewer.pal(name = "GnBu", n = 9)[5:9]
ggplot(dino_genus, aes(x = date, y = Abundance, fill = Order)) +
  facet_grid(factor(depth_layer, levels = c("Surf-ML", "ML-75m")) ~.) +
  geom_bar(position = "fill", stat = "identity") +
  theme(axis.title.x = element_blank()) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  guides(fill = guide_legend(keywidth = 1, keyheight = 1)) + 
  ylab("Relative 18S Read Abundance") +
...
```{r}
ml_75m_counts <- counts_samples %>%
  filter(depth_layer == 'ML-75m')
head(ml_75m_counts)
```
...
depth_asv_presence <- rowSums(ml_75m_counts !=0)
...
boxplot(E1_summary, E2_summary, E3_summary, ylab = "ASV Richness", xlab = 'Epoch', names = c("E1", "E2", "E3"), col = "dark green")
```
...
```{r}
a <- boxplot(surf_sum, depth_sum, names = c("Surface - ML", "ML - 75M"), horizontal = FALSE, xlab = "Depth", ylab = "ASV Richness") 
```
...
```{r}
label <- c("Mixed Layer Depth - 75m      Surface - Mixed Layer", cex = 1.5)
ps_venn(ASV_phyloseq, 'depth_layer', fills = c("light blue", "light green", "aquamarine1"), by = list('depth_layer'), adjust_labels = TRUE, main = label, quantities = list(cex = 4), shape = list(cex = 0.1), labels = FALSE, legend = TRUE)
...
```{r}
ML_taxa <- subset(taxa_df, ASV %in% venn$`ML-75m`)
ML_taxa
```
